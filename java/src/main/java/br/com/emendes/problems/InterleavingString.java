...
public class InterleavingString {
  ...
  private boolean isInterleave(String s1, String s2, String s3, int s1Index, int s2Index, int s3Index, int[][][] dp) {
    ...
    if (s1Index < s1.length() && s1.charAt(s1Index) == s3.charAt(s3Index)) {
      answer = isInterleave(s1, s2, s3, s1Index + 1, s2Index, s3Index + 1, dp);
    }
    if (!answer && s2Index < s2.length() && s2.charAt(s2Index) == s3.charAt(s3Index)) {
      answer = isInterleave(s1, s2, s3, s1Index, s2Index + 1, s3Index + 1, dp);
    }
